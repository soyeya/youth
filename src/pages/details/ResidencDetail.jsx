...
const ResidenceDetails = () => {
  ...
  /**
   * 와일드카드 라우트 `/details/residence/*` 에서 실제 정책명을 추출.
  ...
  const link = `/details/residence/${encodeURIComponent(values)}`;
  ...
  const loadDetail = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/residenceApi`);
      ...
      console.log('ResidencDetail 로드 오류:', err.message || err);
      setNotFound(true);
    } finally {
      finishLoading();
    }
  ...
  const pageHeader = <Title title={'주거 정책 상세'} />;
