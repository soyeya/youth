...
const Residence = () => {
    ...
    const onClick = async() => {
          ...
          if(!data){
            startLoading(`주거관련 불러오는 중...`)
              try{
                    const res = await axios.get('http://localhost:3400/residenceApi');
                    const res_data = res.data.youthPolicyList;
                    console.log('residencedata작동')
                    if(!res_data.length){ //한개인 경우
                        if(res_data.bscPlanPlcyWayNo === '002'){ //002 거주정책이라면
                          ...
                          return  finishLoading(), setData(arryContent01), setContent(arryContent02), setHref(`/details/residence`);
                      }
                    }
                    if(res_data.length > 0){
                      for(let i = 0; i < res_data.length; i++){
                        if(res_data[i].bscPlanPlcyWayNo === '002'){ //002 거주정책이라면
                  ...
                  // setSection('');
                  setHref('');
                  console.log('residence api불러오기', err)
                  return;
             }
             finally{
              finishLoading();
