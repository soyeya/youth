...
const Job = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const filtered = list.filter((item) => item.bscPlanPlcyWayNo === '001');
      ...
      console.log('일자리 데이터 로드 오류:', err.message || err);
      setRawItems([]);
    } finally {
      finishLoading();
    }
  ...
  return (
    <Page
      header={<Title title={'JOB'} backURL={'/'} />}
      footer={
        upDateLogin ? (
